...
class Mask_c(nn.Module):
    ...
    def forward(self, x):
        ...
        _, C_out, _, _ = c_in.size()
        """
        #--------------------------------------------------------------
        # print(context.squeeze().size(), c_in.squeeze().size())
        # np.savez(str(channel) + "_"+ str(C_out), con_ = context[:5].squeeze().cpu().numpy(), attn =  c_in[:5].squeeze().cpu().numpy())
        #--------------------------------------------------------------
        """
        # channel gate
        ...
        """
        # print(mask_c.shape)
        np.savez("mask_"+str(channel) + "_"+ str(C_out), mask = mask_c[:5].squeeze().cpu().numpy())
        #---------------------------------------------------------------
        """
        # norm
