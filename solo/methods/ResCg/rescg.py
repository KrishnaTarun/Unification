...
class BasicBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        x, flag, queue_, norm_1, norm_2, flops = input
        residual = x
        if not flag: #(flag=0 compute but also check if 'ts trianing to avoid appending in queue)
            mask_c, norm_c, norm_c_t = self.mask_c(x) #mask: [N, C_out, 1, 1]
            if self.training:
                queue_.append(mask_c)
        ...
        return (out, flag, queue_, norm_1, norm_2, flops)#return the flag for flexibility
...
class ResDG(nn.Module):
    ...
    def forward(self, x, flag, den_target, lbda, gamma, p, store=None):
        ...
        #add queue
        x = self.layer1((x, self.flag, self.mask_con, norm1, norm2, flops))

        x = self.layer2(x)

        x = self.layer3(x)

        x, _ ,self.mask_con, norm1, norm2, flops = self.layer4(x) #flag is already set

        if flag: #flag==1 then it should be empty by then
            #this should be empty at this point
            assert len(self.mask_con) == 0
