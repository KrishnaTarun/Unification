...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, h, w, eta=8, stride=1, 
        ...
        self.separate_bn = kwargs.get("separate_bn")
        ...
        #create a mask container for each layer
        
        # self.upsample = nn.Upsample(size=(self.height, self.width), mode='nearest')
        # conv 1
        self.conv1 = conv3x3(inplanes, planes, stride)
        print("separate_bn",self.separate_bn )
        #=====================================
        if not self.separate_bn:
            self.bn1 = norm_layer(planes)
        else:#create two bn layer
            #student
            self.bn1_s = norm_layer(planes)
            #teacher
    ...
    def forward(self, input):
        #flag to check for the computation of mask and flops
        
        x, flag, norm_1, norm_2, flops = input
        residual = x
        # print("check training", self.training)
        if not flag: #(flag=0 compute)
            # print("===check flag==", flag)     
            mask_c, norm_c, norm_c_t = self.mask_c(x) #mask: [N, C_out, 1, 1]
        ...
        if not self.separate_bn:
            ...
            if flag: #flag=1 ==> teacher no compute
                out = self.bn1_t(out)
            else: #flag=0==> compute 
                out = self.bn1_s(out)

        out = self.relu(out)
        # print(out.shape, mask_c.shape)
        ...
        #keep norm_1 and norm_2 as it for flag=0 (no computation for the moment)

        #return the flag for flexibility
        return (out, flag, norm_1, norm_2, flops)
...
class ResDG(nn.Module):
    ...
    def __init__(self, block, layers, h=224, w=224, num_classes=1000,
        ...
        if self.cifar:
            ...
            #No need for this as it is a stem layer
            """
                #student
        ...
        # elif kwargs["dataset"]=="imagenet100":
            ...
            # block
            self.height, self.width = h, w
            print("===check size===",self.height, self.width)
            # print("imagenet")
            # conv1
        ...
        self.num_features = 512*block.expansion #(keep it for linear evaluation)
    ...
    def forward(self, x, flag, den_target, lbda, gamma, p, store=None):
        ...
        #residual modules
        # if not self.flag: NOTE keep it for both flag option 
        norm1 = torch.zeros(1, batch_num+1).to(x.device) #used while computing spatial mask, can be ignored
        norm2 = torch.zeros(1, batch_num+1).to(x.device)
        flops = torch.zeros(1, batch_num+2).to(x.device)

        #add queue
        x = self.layer1((x, self.flag, norm1, norm2, flops))

        x = self.layer2(x)

        x = self.layer3(x)

        x, _, norm1, norm2, flops = self.layer4(x) #flag is already set
        # print("here")
        ...
        if flag:
            #no need to return other losses when flag is set to 1 
            return outputs

        # norm and flops
        # norm_s = norm1[1:, 0:batch_num].permute(1, 0).contiguous() #spatial mask
...
if __name__ == "__main__":
    ...
    h = 2
    w = 2
    model = resdg18(h=224, w=224,**{"cifar": False,"separate_bn":True})
    total_params = sum(p.numel() for p in model.parameters())
    
    a = []
    for name, p in model.named_parameters():
        if p.requires_grad:
            if "mask_c" in name.split("."):
                # print(name)
                a.append(p.numel()) 

    print(sum(a), (sum(a)/total_params)*100)
    ...
    output = model(y,  0, den_target, lbda, gamma, p)
    ...
    flops_ori = flops_ori.mean(0).sum() + flops_conv1.mean() + flops_fc.mean()
    flops_real = flops_conv + flops_mask + flops_conv1.mean() + flops_fc.mean()
    print(flops_mask, flops_ori, flops_real/flops_ori)
    # print(output)
